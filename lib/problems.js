...
function maxNonAdjacentSum(nums) {
    ...
    let firstIdx = firstSlice.indexOf(firstMax) - 1;
    let secondIdx = secondSlice.indexOf(secondMax) - 1;
    if (firstIdx < 0) firstIdx = 0;
    if (secondIdx < 0) secondIdx = 0;
    console.log(firstIdx);
    let firstArray = firstSlice.slice(0, firstIdx).concat(firstSlice.slice(firstIdx + 2));
    console.log(firstArray);
    let secondArray = secondSlice.slice(0, secondIdx).concat(secondSlice.slice(secondIdx + 2));
    let firstResult = first + firstMax + maxNonAdjacentSum(firstArray);
    let secondResult = second + secondMax + maxNonAdjacentSum(secondArray);
...
// console.log(maxNonAdjacentSum([4, 14, 1, 4, 3]))
